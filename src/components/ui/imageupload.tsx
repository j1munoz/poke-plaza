...
export default function ImageUploader({
  ...
  const initial: ImageListType = useMemo(
    () => (value ? value.map((u) => ({ dataURL: u })) : []),
    [value],
  );

  const [images, setImages] = useState<ImageListType>(initial);
  useEffect(() => setImages(initial), [initial]);

  const handleChange = (imageList: ImageListType) => {
    setImages(imageList);
    onChange?.(imageList.map((i) => i.dataURL!).filter(Boolean));
  };
